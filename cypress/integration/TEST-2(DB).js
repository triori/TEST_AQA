    ...
    it('Population density', () => {
        ...
        cy.task('Countries', query).then((rows) => {
            for (var i = 0; i < rows.length; i++) {
                rows[i].population / rows[i].area < 50 && rows[i].name === 'USA' ? printResults(rows, i, ' population density is ') :  rows[i].population / rows[i].area > 50 ? printResults(rows, i, ' population density is ') : expect(false).to.equal(true)
            }
